"""
##VERION WITH NO CENTROID CONTROL
...
# ) -> np.ndarray:
#     """Prior factor: error = variable - measurement."""
#     current_centroid_v = values.atVector(this.keys()[0])
#     mass = values.atVector(this.keys()[1])
...
class ForcelessCentralisedControllerCVel(BaseController):
    """
    ...
    def compute_control(
        ...
        # U_c_all, mass_estimate, centroid_velocity_estimate = self._solve_fg(centroid, robots, goal, dt, mass_estimate, centroid_velocity_estimate)
        U_c_all = self._solve_fg(centroid, robots, goal, dt)
    ...
    def _solve_fg(
        ...
        def Vk(j): return gtsam.symbol('V', j)
        ...
        for j in range(N):
            ...
            if j == 0:
                ...
                # if centroid_velocity_estimate is None:
                #     init.insert(kV, np.array([0.0, 0.0, 0.0]))
                # else:
                #     init.insert(kV, centroid_velocity_estimate)
            ...
            # init.insert(kU, u_warm) #no longer doing that
            ...
            #can't be done without forces => we'd have C(t+1) = C(t) + dt*V(t) but the only way we can estimate V(t) is V(t) = (C(t+1)- C(t))/dt so the equations would cancel out
            # graph.add(gtsam.CustomFactor(
